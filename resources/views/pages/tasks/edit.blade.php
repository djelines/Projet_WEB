    ...
    <div class="max-w-3xl mx-auto mt-4 !shadow-xl bg-slate-50 rounded-2xl border-6 dark:bg-[--tw-page-bg-dark]">
        ...
        <form action="{{ route('tasks.update', $task->id) }}" method="POST" class="space-y-4 p-8 bg-white dark:bg-[--tw-page-bg-dark] rounded-xl shadow-lg">
            ...
            <div class="space-y-2">
                ...
                <input type="text" name="title" id="title" value="{{ $task->title }}" class="w-full px-4 py-3 border border-gray-300 rounded-lg shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition duration-200" placeholder="Entrez un titre...">
            </div>

            <!-- Description -->
            <div class="space-y-2">
                <label for="description" class="block text-base font-semibold text-gray-800">Description</label>
                <textarea name="description" id="description" rows="4" class="w-full px-4 py-3 border border-gray-300 rounded-lg shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition duration-200" placeholder="Ajoutez des détails...">{{ $task->description }}</textarea>
            </div>

            <!-- Promos concernées -->
            <div class="space-y-2">
                <label for="cohorts" class="block text-base font-semibold text-gray-800">Promos concernées</label>
                <select name="cohorts[]" id="cohorts" multiple required class="w-full px-4 py-3 border border-gray-300 rounded-lg shadow-sm bg-white dark:bg-[--tw-page-bg-dark] focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition duration-200">
                    @foreach($allCohorts as $cohort)
                    <option value="{{ $cohort->id }}" {{ in_array($cohort->id, $task->cohorts->pluck('id')->toArray()) ? 'selected' : '' }}> {{ $cohort->name }}
                    </option>
                    @endforeach
                </select>
            ...
            <div class="space-y-2">
                ...
                <select name="category" id="category" class="w-full px-4 py-3 border border-gray-300 rounded-lg shadow-sm bg-white dark:bg-[--tw-page-bg-dark] focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition duration-200">
            ...
            <div class="flex flex-row items-end justify-end pt-4 border-gray-200 gap-4">
                <!-- Back -->
                <a href="{{ route('tasks.index') }}" class="bg-purple-700 hover:bg-purple-900 px-4 py-2 rounded-lg !text-white text-sm font-bold transition duration-300 ease-in-out transform hover:scale-105 uppercase">
                    ← Revenir
                </a>

                <!-- Update -->
                <button type="submit" class="!px-4 py-2 !bg-teal-600 !text-white rounded-lg font-semibold text-sm transition duration-300 ease-in-out transform hover:!bg-teal-700 hover:scale-105 uppercase">
                    Mettre à jour →
                </button>
