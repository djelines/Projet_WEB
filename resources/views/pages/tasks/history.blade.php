<x-app-layout>
    ...
    <div class="max-w-5xl mx-auto">
        <div class="bg-white p-6 rounded-xl shadow-lg space-y-6">
            ...
            </h3> @forelse($completedTasks as $task)
            <div class="bg-gray-50 p-4 rounded-lg shadow-sm hover:shadow-md transition duration-300 border-l-4 border-emerald-500">
                <div class="flex justify-between items-start">
                    <div>
                        <h4 class="text-lg font-semibold text-emerald-700">{{ $task->title }}</h4> @if($task->pivot->comment)
                        <div class="mt-2">
                    ...
                    <a href="{{ route('tasks.show', ['task' => $task->id, 'from_history' => true]) }}" class="ml-4 bg-emerald-600 hover:bg-emerald-700 !text-white text-xs font-semibold px-3 py-2 rounded-lg transition transform hover:scale-105">
                            Voir
                        </a>
                </div>
            </div>
