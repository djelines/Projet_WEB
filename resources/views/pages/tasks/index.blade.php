    ...
    <div class="grid lg:grid-cols-2 gap-5 lg:gap-7.5 items-stretch">
        <div class="lg:col-span-2">
            ...
            <form action="{{ route('tasks.index') }}" method="GET" class="inline flex flex-row items-end justify-end mt-6 mb-4 gap-4">
                ...
                <a href="{{ route('tasks.history') }}" class="bg-violet-500 hover:bg-violet-700 !text-white px-4 py-2 rounded-lg text-sm transition duration-300 ease-in-out">Voir mon historique</a> @endcan @can('viewCompletedByStudents', App\Models\Task::class)
            ...
            <div class="flex flex-wrap gap-8 mt-6 justify-center">
                ...
                <div class="max-w-xs bg-white dark:bg-[--tw-page-bg-dark] shadow-lg rounded-lg p-6 relative transform transition duration-300 hover:scale-105 hover:shadow-xl border-2 border-gray-200 flex flex-col w-full sm:w-1/2 lg:w-1/3">
                    ...
                    <!-- Category colours-->
                    @php $categoryColors = [ 'Ménage' => 'bg-blue-50', 'Rangement' => 'bg-purple-50', 'Courses groupées' => 'bg-green-50', 'Courses urgentes' => 'bg-green-50', 'Vaisselle' => 'bg-cyan-50', 'Cuisine' => 'bg-orange-50', 'Achats communs' => 'bg-lime-50', 'Maintenance'
                    => 'bg-gray-50', 'Événement' => 'bg-pink-50', 'Invités' => 'bg-rose-50', 'Réunions' => 'bg-indigo-50', 'Visites des locaux' => 'bg-yellow-50', 'Prise en main des stagiaires' => 'bg-red-50', 'Autre' => 'bg-neutral-50', ]; $color = $categoryColors[$task->category]
                    ?? 'bg-gray-400'; @endphp
                    ...
                    <p class="text-xs text-gray-800 bg-gray-100 px-4 py-2 rounded shadow-sm">
                        <span class="font-semibold text-blue-600">Promotion(s) associée(s) :</span> {{ $task->cohorts->pluck('name')->join(', ') }}
                    </p>
                    @endif
                    ...
                    <div class="flex justify-center space-x-2 mt-4 mb-4">
                        ...
                        <a href="{{ route('tasks.show', $task->id) }}" class="bg-emerald-500 hover:bg-emerald-600 !text-white px-4 py-2 rounded-lg text-sm transition duration-300 ease-in-out transform hover:scale-105">
                                        Valider la Tâche
                                    </a> @elseif(auth()->user()->school()->pivot->role === 'student' && in_array(auth()->user()->id, $task->users->pluck('id')->toArray()))
                        <div class="mt-2">
